...
var Meeting = function (socketioHost) {
    ...
    {
            urls: "turn:turn.napaytravel.com:5349",
            username: "napay1",
            credential: "pruebanapayturn1234"
    }]};
    ...
    function createOffer(participantId) {
        ...
        var onSuccess = function(participantId) {
            return function(sessionDescription) {
                ...
                sessionDescription.sdp = setBandwidthRestriction(sessionDescription.sdp, _maxBandwidth);
    ...
    function setBandwidthRestriction(sdp, bandwidth) {
      var modifier = 'AS';
    ...
    function createChatRoom(formId, inputChatId, messageBoxId) {
        console.log("Chat room created");
        _defaultChannel.on('chatMessage', function(msg){
            console.log(msg);
          $('#'+messageBoxId).append($('<li>').text(msg));
        });

        $('#'+formId).submit(function(e){
          e.preventDefault();
          _defaultChannel.emit('chatMessage', $('#'+inputChatId).val());
          $('#'+messageBoxId).append($('<li>').text( $('#'+inputChatId).val()));
          $('#'+inputChatId).val('');
          return false;
        });
    }
